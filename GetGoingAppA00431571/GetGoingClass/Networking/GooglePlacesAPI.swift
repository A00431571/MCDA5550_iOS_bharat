...
//  GetGoingClass
//
//  Created by Bharat Bhushan Verma on 2019-03-31.
    ...
    class func requestPlaces(_ query: String, completion: @escaping(_ status: Int, _ json: [String: Any]?) -> Void) {
        ...
        if let url = urlComponents.url {
            NetworkingLayer.getRequest(with: url, timeoutInterval: 500) { (status, data) in
                ...
                if let  ponseData = data,
                    let jsonResponse = ((try? JSONSerialization.jsonObject(with: responseData, options: .allowFragments) as? [String: Any]) as [String : Any]??) {
